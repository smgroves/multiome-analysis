---
title: "Single Cell SCLC Archetypes for Full Dataset"
output:
  html_document:
    df_print: paged
---
...
data <- readRDS("/Users/smgroves/Box/multiome_data/TKO_final.rds") #loaded from Debbie's RDS file in Box
...
# Reductions from Debbie
```{r reductions}
data@reductions$wnn.umap
data@reductions$harmony_1
data@reductions$harmony_2
...
pca_1 <- data[["harmony_2"]]@cell.embeddings[,1]
pca_2 <- data[["harmony_2"]]@cell.embeddings[,2]
pca_3 <- data[["harmony_2"]]@cell.embeddings[,3]

# Visualize what headings are called so that you can extract them to form a dataframe
# Embeddings(object = data, reduction = "harmony_2")
plot.data <- FetchData(object = data, vars = c("harmony2_1", "harmony2_2", "harmony2_3", "identity"))
plot.data$label <- paste(rownames(plot.data))
plot_ly(data = plot.data, 
        x = ~harmony2_1, y = ~harmony2_2, z = ~harmony2_3, 
        color = ~identity, 
        colors = hsv(seq(0,1 - 1/14,length.out = 14), 0.5 , 1),
...
VizDimLoadings(data, dims = 1:3, reduction = "harmony_2")

```

# Archetypal Analysis

We will now apply the archetype analysis to the scRNAseq data. We will use harmony_2, which was run on the SCT assay (transformed RNA counts), instead of PCA. We'll call the data `x_harmony`.
...
x_harmony <- Embeddings(object = data, reduction = "harmony_2")
x_harmony <- t(x_harmony)

write.csv(x_harmony, file="./ParetoTI/x_harmony2.csv")


#x_harmony <- x_harmony[1:15,] #use all harmony components

```

``` {r}
arc_ks = k_fit_pch(x_harmony, ks = 2:8, check_installed = T,
...
```{r}
i = 6
arc <- fit_pch(x_harmony, noc = i, delta = 0, conv_crit = 1e-04, maxiter = 500)

start = Sys.time()
pch_rand = randomise_fit_pch(x_harmony, arc_data = arc,
                             n_rand = 1000,
...
# 1: 6   varexpl 0.4995413491   0.001
# 2: 6   t_ratio 0.0003385489   0.105
...
``` {r}
arc <- fit_pch(x_harmony, noc = 6, delta = 0, conv_crit = 1e-04, maxiter = 500)

cols <- hsv(seq(0,1 - 1/16,length.out = 16), 0.5 , 1)

plot_arc(arc_data = arc, data = x_harmony,
...
arc_rob = fit_pch_bootstrap(x_harmony, n = 200, sample_prop = .8, seed = 2543, delta = 1, noc = 5)
...
reconstruct_from_pca <- function(x, loadings, arc_XC, nComp = 15){
    ...
    return(Xhat)}
x_recon <- reconstruct_from_pca(x, loadings, x_harmony)
...
plot_arc(arc_data = arc, data = x_harmony,
                   which_dimensions = 1:2,colors = cols,
                  data_lab = as.character(Idents(data))) + theme_bw()
p_pca = plot_arc(arc_data = arc, data = x_harmony, 
                 which_dimensions = 1:3, line_size = 1.5,
                 colors = cols,
                  data_lab = as.character(Idents(data)),
                 text_size = 60, data_size = 2) 
plotly::layout(p_pca, title = "Archetypes for Harmony Data")
htmlwidgets::saveWidget(p_pca, "./ParetoTI/archetypes.html")
# 
# p_pca = plot_arc(arc_data = arc_ave, data = x_harmony,
#                  which_dimensions = 1:3, line_size = 1.5,
...
arc_umap = arch_to_umap(arc, x_harmony, which_dimensions = 1:2)
...
data_attr = merge_arch_dist(arc_data = arc_ave, data = x_harmony, 
