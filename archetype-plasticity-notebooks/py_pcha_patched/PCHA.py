...
from py_pcha_patched.furthest_sum import furthest_sum
...
def PCHA(X, noc, I=None, U=None, delta=0, verbose=False, conv_crit=1E-6, maxiter=500):
    ...
    def C_update(X, XSt, XC, SSt, C, delta, muC, mualpha, SST, SSE, niter=1):
        ...
        for k in range(niter):
            ...
            SSE_old = SSE
            if delta != 0:
                g = (np.diag(CtXtXC * SSt).T / alphaC -
                     np.sum(C.A * XtXSt.A)) / (SST * J)
                alphaC_old = alphaC
                while True:
                    ...
                    SSE = SST - 2 * np.sum(XCt.A * XSt.A) + \
                        np.sum(CtXtXC.A * SSt.A)
