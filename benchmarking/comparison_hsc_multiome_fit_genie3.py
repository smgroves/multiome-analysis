...
def main():
    ...
    edge_list = pd.read_csv(f"{DATA_DIR}/candidate_network_chea_real.csv", header=None, names=["source", "target"])
    ...
    out_dir = f"{BRCD}/validation/genie3_validation/accuracy_plots"
    ...
    importance.to_csv(f"{BRCD}/rules/genie3_importance_matrix.csv")
    print(f"[write] {len(fitted_regulators)}/{len(genes)} genes fit -> {out_dir}")
    print(f"[write] importance matrix -> {BRCD}/rules/genie3_importance_matrix.csv")
