# Phase 2, item 3 hyperparameter test run: identical to main_all_data_remove_selfloops_6667
# copy.py (original 228-edge network, same data source/split) except using the
# "global-reference" normalization prototype (build_global_reference_norm.py) instead of
# the quantile-clip node_normalization=0.3 scheme -- see BoBa-T_hyperparameters.md sec 4.
# That script pre-scales every gene to [0,1] via a logistic referenced to a SHARED,
# network-wide scale constant (median of all genes' raw std, K=0.2) rather than that
# gene's own min/max/quantiles, so genuinely low-spread genes (BACH2, ZEB1, etc, per
# screen_bimodality_spread.py) stay compressed near 0.5 instead of being artificially
# stretched to fill the same range as a strongly bimodal gene. Data is already scaled on
# disk, so node_normalization is set to None here (pass-through in bb.load.load_data) --
# no BoBa-T source change. Does not touch or overwrite anything under 6667/, 6668/, or 6669/.
import random
import time
...
if __name__ == "__main__":
    ...
    node_normalization = None  # data is pre-scaled by build_global_reference_norm.py; pass-through
    ...
    fname = "combined"
    notes_for_log = (
        "6670: same network/data-source/split as 6667, but using the global-reference "
        "normalization prototype (logistic squash referenced to a shared network-wide "
        "scale constant, K=0.2, build_global_reference_norm.py) instead of the "
        "quantile-clip node_normalization=0.3 scheme, to test whether avoiding "
        "per-gene-forced [0,1] spread improves fit robustness."
    )
    ## Set paths
    ...
    data_path = "/data/adata_imputed_combined_v3_RORA_RORB_ave_globalnorm.csv"
    t1 = False
    data_t1_path = None
