...
cellID_table = f'data/human_tumor_MSK/{sample}_clusters.csv'
...
data_test_t0_path = f'data/human_tumor_MSK/adata_{sample}.csv'
...
clusters = bb.utils.get_clusters(data_test_t0_path, is_data_split=False,cellID_table=f"{dir_prefix}/{cellID_table}",cluster_header_list=cluster_header_list)
...
if "RORA" in data_test_t0.index and "RORB" in data_test_t0.index:
    data_test_t0.loc["RORA_RORB"] = np.mean(data_test_t0.loc["RORA"], data_test_t0.loc["RORB"])
