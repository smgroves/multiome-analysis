...
def main():
    ...
    for label, start_idx in ORGANOID_STARTS.items():
        ...
        curves_by_condition = {}
        for condition, walks in [("unperturbed", walks_unpert), ("RORA_RORB_kd", walks_kd)]:
        ...
        if label in NE_STARTING_LABELS:
            ...
            for legend_label, curves, mean_curve, color, linestyle in kd_series + unpert_series:
                for curve in curves:
            ...
            plt.title(f"Distance to archetype clusters over time\norganoid '{label}' start: knockdown (color) vs. unperturbed negative control (grey)")
            ...
            stats_text = (
                ...
                f"Knockdown: mean step {kd_row['mean_step_closest_to_Arc1']:.0f} vs. {kd_row['mean_step_closest_to_nonNE']:.0f}; "
            ...
            for ext in ["png", "pdf"]:
                plt.savefig(f"{OUT_DIR}/walk_temporal_distance_{label.replace(' ', '_')}_combined.{ext}", dpi=150, bbox_inches="tight")
            plt.close()

    ordering_df = pd.DataFrame(ordering_rows)
    ordering_df.to_csv(f"{OUT_DIR}/walk_temporal_ordering.csv", index=False)
    pd.set_option("display.width", 160)
    print(ordering_df.to_string(index=False))

    print(f"\nWrote {OUT_DIR}/walk_temporal_ordering.csv and "
          f"walk_temporal_distance_<label>_combined.{{png,pdf}} for {NE_STARTING_LABELS}")
