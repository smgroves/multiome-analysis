...
if __name__ == "__main__":
    ...
    if validation_averages:
        ...
        bb.plot.plot_aucs(
            VAL_DIR, save=True, show_plot=False
    ...
    if find_average_states:
        ...
        average_states = bb.tl.find_avg_states(
            binarized_data_t0, nodes, save_dir=ATTRACTOR_DIR
        )
    ...
    if find_attractors:
        ...
        # Plot average state for each subtype
        bb.plot.plot_attractors(
            f"{ATTRACTOR_DIR}/attractors_unfiltered.txt", save_dir=""
        )
    ...
    if filter_attractors:
        ...
        average_states_df = pd.read_csv(
            f"{AVE_STATES_DIR}/average_states.txt", sep=",", header=0, index_col=0
        )
        ...
        def plot_attractors_clustermap(fname, sep=","):
            ...
            g = sns.clustermap(
                att.T.reset_index().drop("index", axis=1).T,
                linecolor="lightgrey",
                linewidths=1,
                figsize=(20, 12),
                cbar_pos=None,
            ...
            for i in lut.keys():
                markers.append(
                    plt.Line2D([0, 0], [0, 0], color=lut[i], marker="o", linestyle="")
                )
            lgd = plt.legend(
                markers,
                lut.keys(),
            ...
            plt.tight_layout()
            plt.savefig(
                f"{fname.split('.')[0]}_clustered.pdf",
    ...
    if perturbations:
        ...
        perturb_dict, full = bb.utils.get_perturbation_dict(
            attractor_dict,
            perturbations_dir,
            significance="both",
            save_full=False,
        ...
        bb.plot.plot_perturb_gene_dictionary(
            perturb_gene_dict,
            full,
            perturbations_dir,
            show=False,
