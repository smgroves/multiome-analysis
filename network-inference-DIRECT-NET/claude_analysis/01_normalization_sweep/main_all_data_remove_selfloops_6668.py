# Hyperparameter test run: identical to main_all_data_remove_selfloops_6667 copy.py
# (network based off networks/feature_selection/DIRECT-NET_network_2020db_0.1/
# combined_DIRECT-NET_network_2020db_0.1_Lasso_wo_sinks.csv with RORA/RORB combined into
# one node) except node_normalization=0.4 instead of 0.3, to test whether widening the
# per-gene quantile-clip window (more cells pinned to exact 0/1) pulls fitted rules away
# from the undocumented 1-max(heat) pull-to-0.5 pseudo-observation term in bb.tl.get_rules
# -- see /Users/xpz5km/.claude/plans/it-is-looking-like-elegant-patterson.md for the full
# mechanism writeup. Does not touch or overwrite anything under 6667/.
import random
import time
...
if __name__ == "__main__":
    ...
    fit_rules = False  # already fit and saved in a prior run of this script; reuse rules_6668.txt
    run_validation = True
    ...
    node_normalization = 0.4  # 6667 uses 0.3; this is the only parameter changed
    ...
    fname = "combined"
    notes_for_log = (
        "6668: same network/data/split as 6667, but node_normalization=0.4 (vs 0.3) to "
        "test whether widening the quantile-clip window improves external validation on "
        "the shGFP organoid null condition."
    )
    ## Set paths
