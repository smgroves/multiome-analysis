...
# True discovered attractor states (0/53 genes with >50% flip probability -- confirmed
# genuine fixed points), NOT archetype averages -- see FINDINGS.md. 7788149757105118 is
# one of Arc_5/NE1's 11 filtered attractor members; 95331434180192 is a true attractor
# SHARED by Generalist_nonNE and Arc_4/nonNE1 (confirmed identical state in both basins).
SPECS = [
    ("gemm_ne1_start", "hexagon_GEMM_NE1_true_attractor", "GEMM NE1 (true attractor state)", GEMM_WALK_DIR, 7788149757105118,
     "results_RORA_RORB_kd.csv", "RORA_RORB knockdown", "tab:purple"),
    ("gemm_nonne_start_oe", "hexagon_GEMM_nonNE_true_attractor", "GEMM nonNE (true attractor, shared by Generalist_nonNE/Arc_4)", GEMM_WALK_DIR, 95331434180192,
...
def main():
    ...
    for out_subdir, out_name, label, walk_dir, start_idx, pert_file, pert_display, pert_color in SPECS:
        ...
        note = (
            "Note: start is one of bobaT's own dynamically-discovered attractor states (0/53 genes with >50% flip probability --\n"
            "a genuine fixed point, confirmed directly), NOT the archetype's average state (compare to the companion plots without\n"
            f"this suffix). Hexagon vertices + the 2 circles are GEMM's own fitted archetype average states, projected by Hamming\n"
