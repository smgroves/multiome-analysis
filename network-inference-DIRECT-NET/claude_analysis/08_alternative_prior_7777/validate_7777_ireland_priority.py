"""Validate the corrected 7777 refit (remove_selfloops=True, matching 6667) against just
...
def main():
    ...
    for name in IRELAND_ORDER:
        ...
        row = {
            ...
            "baseline_r2": baseline_r2, "r2_7777": stats["r2"].mean(),
            "baseline_r2_median": baseline_r2_median, "r2_7777_median": stats["r2"].median(),
            "baseline_auc": baseline_auc, "auc_7777": stats["roc_auc_score"].mean(),
        }
        rows.append(row)
        print(f"{name} (n={row['n_cells']}): R2 mean {baseline_r2:.3f}->{row['r2_7777']:.3f}, "
              f"median {baseline_r2_median:.3f}->{row['r2_7777_median']:.3f}, "
              f"AUC {baseline_auc:.3f}->{row['auc_7777']:.3f}")
        pd.DataFrame(rows).to_csv(f"{DIR_PREFIX}/claude_analysis/08_alternative_prior_7777/7777_refit_vs_6667_ireland.csv", index=False)

    result = pd.DataFrame(rows)
    pd.set_option("display.width", 160)
    print("\n=== Ireland 2025 comparison (corrected 7777 refit) ===")
